...
def talker():
  ...
  nbpulse_gpiston = 1300 # better
#  nbpulse_gpiston = 1400 # test CK
  nbpulse_quarterout = nbpulse_max \
		- int((nbpulse_max-nbpulse_gpiston)/4.)

  # target piston positions and time intervals
  nbpulse_steps = [nbpulse_gpiston, nbpulse_max, nbpulse_gpiston] #nb pulse
#  nbpulse_steps = [nbpulse_gpiston, nbpulse_max, 1000] #nb pulse
  nbpulse_steps = [0, 6000, 1300] #nb pulse
  dt_steps =      [20             , 30         ,  60] # min
