...
volatile unsigned short motor_flag_regulation = 0;
volatile signed int motor_speed_max = 50;                     // test
...
volatile unsigned short flag_optical_fork = 1; // test
...
volatile unsigned short debug_period = 15;
volatile unsigned short flag_debug = 0;
volatile unsigned int optical_counters_debug[4] = {0,0,0,0}; // test
volatile unsigned short flag_count=0;
...
void i2c_write_data_to_buffer(unsigned short nb_tx_octet){
    ...
    case 0x00:
        SSPBUF = optical_counters_debug[0];
        //SSPBUF = nb_pulse;
        //SSPBUF = mytimer;
        break;
    case 0x01:
        SSPBUF = optical_counters_debug[0] >> 8;
        //SSPBUF = nb_pulse >> 8;
        //SSPBUF = (mytimer >> 8);
        break;
    case 0x02:
        ...
        break;
    case 0x03:
        SSPBUF = optical_counters_debug[1];
        //SSPBUF = position_set_point;
        //SSPBUF = mytimer;
        break;
    case 0x04:
        SSPBUF = optical_counters_debug[1] >> 8;
    ...
    case 0x08:
        ...
        //SSPBUF = time_zero_shift_error;   // switch to 56 but not athe same time
        //SSPBUF = delay_release_torque_cpt; // 0 then 1 and 0 again then 57
        //SSPBUF = delay_release_torque; // 10 then 56
        //SSPBUF = flag_regulation; // 1 (reset) then 0 (regulation)
        break;
    case 0x09:
    ...
    case 0xA0:
        //SSPBUF = error;
        SSPBUF = optical_counters_debug[2];
        //SSPBUF = nb_pulse_debug;
        break;
    case 0xA1:
        //SSPBUF = (error >> 8);
        SSPBUF = optical_counters_debug[2] >> 8;
        //SSPBUF = nb_pulse_debug >> 8;
        break;
    case 0xB0:
        SSPBUF = optical_counters_debug[3];
        //SSPBUF = battery_voltage;
        break;
    case 0xB1:
        SSPBUF = optical_counters_debug[3] >> 8;
...
void read_optical_fork_debug(){
    ...
    //optical_counter = 0;
    for (i=0; i<4; i++){
        if (optical_counters[i]>1000){
            optical_counters_debug[i]++;
...
void init_io(){
    ...
    // test: uncomment next 3 lines, result: cannot capture nb_pulse anymore
...
void main(){
  ...
  *   -> Watchdog Time Postscale : 1:256 (32768/31000 = environ 1Hz)
    ...
    */
    ...
    //INTCON2.RABIP = 0; // RA and RB Port Change Interrupt Priority bit, low priority
    //IOCA.IOCA0 = 1; // Interrupt-on-change enabled sur RA0
    //IOCA.IOCA1 = 1; // Interrupt-on-change enabled sur RA1
    ...
    while(1){
        ...
        if (flag_debug==1){
            if(debug_counter<debug_period){
                LED2=1;
                set_motor_cmd(MOTOR_STOP - motor_speed_max); // In
                flag_debug=0; 
                flag_count=1;
            }
            else if(debug_counter<2*debug_period){
                LED2=0;
                set_motor_cmd_stop();
                flag_debug=0;
                flag_count=0;
                ...
                flag_debug=0;
                for (i=0; i<4; i++){
                    optical_counters_debug[i] = 0;
                }
            }            
            else if(debug_counter<3*debug_period){
                LED2=1;
                set_motor_cmd(MOTOR_STOP + motor_speed_max); // Out
                flag_debug=0;
                flag_count=1;
            }
            else if(debug_counter<4*debug_period){
                LED2=0;
                set_motor_cmd_stop();
                flag_debug=0;
                flag_count=0;
